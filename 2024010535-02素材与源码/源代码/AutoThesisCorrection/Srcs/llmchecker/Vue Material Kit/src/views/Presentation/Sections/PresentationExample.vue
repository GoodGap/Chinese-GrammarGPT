      ...
      <div class="row">
        <div class="row justify-content-center text-center my-sm-5">
          <div class="col-lg-6">
            ...
            <h2 class="text-dark mb-0">丰富的功能支持</h2>
            <p class="lead">
              我们不仅支持了常见的文本编辑功能，同时集成了错误提示与纠正
            </p>
          </div>
        </div>
      </div>
      ...
      <div v-for="({ heading, description, items }, index) in data"
        ...
        <div :class="`${col2 ?? 'col-lg-9'}`">
          <div :class="`row ${index != 0 ? 'mt-3' : ''}`">
            <div class="col-md-4 mt-md-0" v-for="{ image, title, pro } in items" :key="title">
              <ExampleCard class="min-height-160 shadow-lg mt-4" :image="image" :title="title" :pro="pro" />
            </div>
          </div>
        </div>
      </div>
