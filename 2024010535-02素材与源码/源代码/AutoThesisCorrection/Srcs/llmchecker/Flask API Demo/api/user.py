# Todo : 重新设计数据表，考虑其他需要增加的用户字段信息，以及增加邮箱的格式验证以及是否已经注册验证
...
def get_user(username):
    """获取某个用户信息"""
    sql = "SELECT * FROM userdata WHERE username = '{}'".format(username)
    data = db.select_db(sql)
...
def user_register():
    ...
    email = request.json.get("email", "").strip()  # 邮箱
    address = request.json.get("address", "").strip()  # 地址，默认为空串
    if username and password and telephone and email:  # 注意if条件中 "" 也是空, 按False处理
        sql1 = "SELECT username FROM userdata WHERE username = '{}'".format(username)
        res1 = db.select_db(sql1)
        print("查询到用户名 ==>> {}".format(res1))
        sql2 = "SELECT telephone FROM userdata WHERE telephone = '{}'".format(telephone)
        ...
        #     return jsonify({"code": 2003, "msg": "输入的性别只能是 0(男) 或 1(女)！！！"})
            ...
            sql3 = (
                "INSERT INTO userdata(username, password, role, telephone, email, address) "
                "VALUES('{}', '{}', '1', '{}', '{}', '{}')".format(
                    username, password, telephone, email, address
                )
            )
            db.execute_db(sql3)
        ...
        return jsonify({"code": 2001, "msg": "用户名/密码/手机号/邮箱不能为空，请检查！！！"})
...
def user_login():
    ...
    if username and password:  # 注意if条件中空串 "" 也是空, 按False处理
        ...
        sql2 = (
            "SELECT * FROM userdata WHERE username = '{}' and password = '{}'".format(
...
def user_update(id):  # id为准备修改的用户ID
    ...
    new_email = request.json.get("email", "").strip()  # 新的邮箱
    new_address = request.json.get("address", "").strip()  # 新的联系地址，默认为空串
    if (
        admin_user and token and new_password and new_telephone and new_email
    ):  # 注意if条件中空串 "" 也是空, 按False处理
        # if not (new_sex == "0" or new_sex == "1"):
        #     return jsonify({"code": 4007, "msg": "输入的性别只能是 0(男) 或 1(女)！！！"})
        if not (
            len(new_telephone) == 11 and re.match("^1[3,5,7,8]\d{9}$", new_telephone)
        ):
            ...
            if redis_token:
                if redis_token == token:  # 如果从redis中取到的token不为空，且等于请求body中的token
                    sql1 = "SELECT role FROM userdata WHERE username = '{}'".format(
                        admin_user
                    )
                    res1 = db.select_db(sql1)
                    ...
                    if user_role == 0:  # 如果当前登录用户是管理员用户
                        sql2 = "SELECT * FROM userdata WHERE id = '{}'".format(id)
                        res2 = db.select_db(sql2)
                        print("根据用户ID 【 {} 】 查询到用户信息 ==>> {}".format(id, res2))
                        sql3 = "SELECT telephone FROM userdata WHERE telephone = '{}'".format(
                            new_telephone
                        )
                        ...
                        if not res2:  # 如果要修改的用户不存在于数据库中，res2为空
                            return jsonify(
                                {"code": 4005, "msg": "修改的用户ID不存在，无法进行修改，请检查！！！"}
                            )
                        elif res3:  # 如果要修改的手机号已经存在于数据库中，res3非空
                            return jsonify(
                                {"code": 4006, "msg": "手机号已被注册，无法进行修改，请检查！！！"}
                            )
                        else:
                            ...
                            sql3 = (
                                "UPDATE userdata SET password = '{}', telephone = '{}', email = '{}', address = '{}' "
                                "WHERE id = {}".format(
                                    new_password,
                                    new_telephone,
                                    new_email,
                        ...
                        return jsonify(
                            {"code": 4004, "msg": "当前用户不是管理员用户，无法进行操作，请检查！！！"}
                        )
                else:
        ...
        return jsonify({"code": 4001, "msg": "管理员用户/token口令/密码/手机号/邮箱不能为空，请检查！！！"})


@user_blueprint.route("/delete/user/<string:username>", methods=["POST"])
    ...
    if admin_user and token:
        ...
        if redis_token:
            if redis_token == token:  # 如果从redis中取到的token不为空，且等于请求body中的token
                ...
                user_role = res1[0]["role"]
                if user_role == 0:  # 如果当前登录用户是管理员用户
                    sql2 = "SELECT * FROM userdata WHERE username = '{}'".format(
                        username
                    )
                    res2 = db.select_db(sql2)
                    print(sql2)
                    ...
                    if not res2:  # 如果要删除的用户不存在于数据库中，res2为空
                        ...
                        return jsonify(
                            {
                                "code": 3006,
                                "msg": "用户名：【 {} 】，该用户不允许删除！！！".format(username),
                            }
                        )
                    else:
                        sql3 = "DELETE FROM userdata WHERE username = '{}'".format(
                            username
                        )
                        db.execute_db(sql3)
