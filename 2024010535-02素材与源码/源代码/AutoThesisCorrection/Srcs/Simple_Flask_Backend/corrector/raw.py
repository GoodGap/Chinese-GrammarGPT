...
from corrector.util import paragraphs
from corrector.util import smart_open
from corrector.levenshtein import equals_ignore_whitespace_casing
from corrector.levenshtein import levenshtein_matrix, edit_graph, merge_graph, transitive_arcs
import corrector.levenshtein as levenshtein
import jieba
...
class LLMCorrector():
    def __init__(self,model=None,tokenizer=None):
        ...
        """
        pynvml.nvmlInit()
        self.model=model if model!=None else self.__ModelInitialize('/home/grammar/Paper_Corrector/backend_interact/mymodel','/home/grammar/Paper_Corrector/backend_interact/CGEC-chatglm2-6b-pt-128-2e-2/checkpoint-3000/pytorch_model.bin')#如果没有指定模型，此函数重新部署模型，在这里修改参数。
        self.tokenizer=tokenizer if tokenizer!=None else self.__TokenizerInitialize('/home/grammar/Paper_Corrector/backend_interact/mymodel')#如果没有加载分词器，则重新加载分词器，路径在此处修改。
    ...
    def __ModelInitialize(self,model_params_path,finetune_params_path):
        ...
        if torch.cuda.is_available():
            ...
            for i in range(device_count):# 这里遍历所有的GPU找能用的，实验室配给我们5，6号卡可能要改成(5,7)
                ...
                if(meminfo.free/1024**3>=16):    #chatglm-6b占用显存11-12G，这里找至少13G可用显存的GPU，要是资源实在紧张可以把这个改成'>=12'.
                    print(f'Device {i} is available!')
                    device = torch.device(f"cuda:{i}")  # 获取当前GPU设备#这里用完之后记得改回i
        ...
        model = model.to(device)  # 将模型移动到当前GPU设备上
        model = model.eval()
